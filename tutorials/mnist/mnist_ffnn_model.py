...
def inference(images, hidden1_units, hidden2_units):
    ...
    with tf.name_scope('hidden1'):
        ...
        # with the first dim representing the number of units in the layer from which the weights connect 
        # and the second dim representing the number of units in the layer to which the weights connect. 
        # For the first layer, named hidden1, the dimensions are [IMAGE_PIXELS, hidden1_units] 
        # because the weights are connecting the image inputs to the hidden1 layer. 
        ...
        weights = tf.Variable(tf.truncated_normal([IMAGE_PIXELS, hidden1_units], 
                              ...
                              name='weights')
        # Then the biases are initialized with tf.zeros to ensure they start with all zero values, 
        # and their shape is simply the number of units in the layer to which they connect.
    ...
    # Linear
    # The logits tensor that will contain the output is returned.
...
def loss(logits, labels):
    ...
    # Then, a tf.nn.sparse_softmax_cross_entropy_with_logits op is added to automatically produce 1-hot labels 
